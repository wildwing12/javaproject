...
--학생(profno) -교수(profno)
select s.studno, s.name sname, d.deptno, d.dname, p.profno, 
p.name pname, s.tel, s.img_path
from student s, department d, professor p
where s.deptno1=d.deptno and s.profno=p.profno;
...
p.name pname, s.tel, s.img_path
from student s, department d, professor p
where s.deptno1=d.deptno and s.profno=p.profno and s.studno=9711;

select * from student;

--9711학생 같은 경우는 지도 교수가 아직 배정 되지 않아 데이터가 출력이 안된다.
--따라서 outer join 을 해주어야 한다.
...
--outer join 방식(해당 데이터가 없더라도 나머지 데이터는 출력 가능하게 해줌)
select studno, s.name sname, p.name pname
...
--수강정보 ex)9411학생의 수강 정보
select st.studno, st.name sname, sb.subject_name, p.name pname, sb.point, l.grade
 from student st, lecture l, subject sb, professor p
 where st.studno=l.studno 
 and l.subject_code=sb.subject_code 
 and p.profno=sb.profno and  st.studno=9411;